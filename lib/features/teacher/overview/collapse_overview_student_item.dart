  ...
  Widget build(BuildContext context) {
    ...
    return Container(
          ...
          richMessage: WidgetSpan(
              ...
              child: Column(
                ...
                children: [
                  RichText(
                    text: TextSpan(
                      text: cubit.students![index].title,
                      style: TextStyle(
                          fontWeight:
                          FontWeight.w600,
                          fontSize:
              ...
              decoration: BoxDecoration(
                color: cubit.students![index].color,
                  borderRadius: BlocProvider.of<DropdownCubit>(context).state % 2 == 0
                      ? BorderRadius.horizontal(left: Radius.circular(Resizable.padding(context, 5)))
                      : BorderRadius.only(topLeft: Radius.circular(Resizable.padding(context, 5)))
              ),
              child: Image.asset('assets/images/ic_${cubit.students![index].icon}.png'),
            ),
          )
        ), name: Text(
      ...
      color: const Color(0xff131111), fontWeight: FontWeight.w500
        ...
        attend: cubit.stdAttends == null || cubit.stdAttends!.isEmpty ? Transform.scale(
          scale: 0.75, child: const Center(child: CircularProgressIndicator())) : CircleProgress(
          title: '${(cubit.stdAttends![index]*100).toStringAsFixed(0)} %',
          lineWidth: Resizable.size(context, 3),
          percent: cubit.stdAttends![index],
          radius: Resizable.size(context, 16),
          fontSize: Resizable.font(context, 14),
        ),
        submit: cubit.stdHomeworks == null ? Transform.scale(
            scale: 0.75, child: const Center(child: CircularProgressIndicator())) : CircleProgress(
          title: '${(cubit.stdHomeworks![index]*100).toStringAsFixed(0)} %',
          lineWidth: Resizable.size(context, 3),

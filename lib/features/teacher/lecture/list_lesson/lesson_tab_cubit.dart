...
class LessonTabCubit extends Cubit<int> {
  ...
  List<String?>? listSpNoteForTeacher;
  ...
  load(int classId, DataCubit dataCubit) async {
    ...
    if (temp.isEmpty) {
      ...
      }
      {
        ...
        List<TeacherModel> teachers = await FireBaseProvider.instance
            .getListTeacherByListId(listTeacherId);
        List<TeacherModel?> listTeacher = [];
        for (var i in listLessonResultTemp) {
          if (i != null) {
            listTeacher.add(teachers
                .firstWhere((element) => element.userId == i.teacherId));
          } else {
        ...
        List<StudentModel> students = classModel2.students!
        ...
        this.listSpNoteForTeacher = listSpNoteForTeacher;

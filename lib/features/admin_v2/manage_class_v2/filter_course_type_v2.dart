...
class FilterCourseTypeAdminV2 extends StatelessWidget {
  FilterCourseTypeAdminV2(this.cubit,{super.key, required this.classCubit}) : selectCubit = SelectFilterCubit();
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<SelectFilterCubit, int>(
        bloc: selectCubit..loadCourse(cubit.filter[AdminFilter.course] == null ? [FilterClassCourse.general] :cubit.filter[AdminFilter.course]!),
        builder: (c,s){
          return  Container(
                ...
                child: Material(
                  ...
                  child: PopupMenuButton(
                      onCanceled: (){
                        ...
                        cubit.update(AdminFilter.course, selectCubit.convertCourse());
                      },
                      itemBuilder: (context) => [
                        ...selectCubit.listCourse.map((e) => PopupMenuItem(
                            padding: EdgeInsets.zero,
                            child: BlocProvider(create: (c)=>CheckBoxFilterCubit(selectCubit.listSelect[selectCubit.listCourse.indexOf(e)]),child: BlocBuilder<CheckBoxFilterCubit,bool>(builder: (cc,state){
                              return CheckboxListTile(
                                controlAffinity: ListTileControlAffinity.leading,
                                title: Text(e.title),
                                value: state,
                                onChanged: (newValue) {
                                  selectCubit.listSelect[selectCubit.listCourse.indexOf(e)] = newValue!;

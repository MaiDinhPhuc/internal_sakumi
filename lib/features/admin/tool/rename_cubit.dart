      ...
      if (drive.error == null && drive.classModel != null) {
        ...
        for (var i in lessonResults) {
          if (i.classId == drive.classModel!.classId) {
            ...
            if (epochLesson > 0) {
              DateTime last = DateTime.fromMillisecondsSinceEpoch(epochVideo);
              DateTime first = DateTime.fromMillisecondsSinceEpoch(epochLesson);
              Duration duration = last.difference(first);
              int temp = duration.inHours;
              if (temp <= 17 && temp >= 0) {
                lessonId = i.lessonId;
                date = DateFormat('dd/MM/yyyy HH:mm:ss').format(first);
                count++;
              }
            }
            // else {
            ...
            //   }
            // }
            // debugPrint('========> epoch date date $date');
          }
        }
            ...
            if (drive.classModel!.customLessons.isNotEmpty) {
              for (var i in drive.classModel!.customLessons) {
                ...
                if (id == i['custom_lesson_id']) {
                  ...
                  debugPrint(
                      '==========> ind 1 $ind -- ${drive.lessonId} -- ${drive.name} -- ${drive.classModel!.classId}');
                  break;
                }
              }
            } else {
